...
class fieldIntMatrix:
    def __init__(self, intMatrix, prime : int, power : int):
        self.dim = (len(intMatrix), len(intMatrix[0]))
        self.primeBase = prime
        self.primePower = power
        self.fieldSize = prime ** power
        zero = fieldInt(0, prime, power)
        self.matrix = [[fieldInt(intMatrix[i][j], prime, power, zero.prodPol) for j in range(self.dim[1])] for i in range(self.dim[0])]

    @staticmethod
    def matrixEye(size : int, prime : int, power : int):
        """
        ...
        return fieldIntMatrix(I, prime, power)
    ...
    def scalarProd(self, scalar : int):
        ...
        return fieldIntMatrix(result, self.primeBase, self.primePower)
    ...
    def T(self):
        ...
        return fieldIntMatrix(selfT, self.primeBase, self.primePower)
    ...
    def __add__(self, other):
        ...
        if self.dim != other.dim:
            return fieldIntMatrix(sumMatrix, self.primeBase, self.primePower)
        for i in range(n):
            for j in range(m):
    ...
    def __neg__(self):
        ...
        return fieldIntMatrix(negSelf, self.primeBase, self.primePower)
    ...
    def __mul__(self, other):
        ...
        # Resulting Matrix
        zero = fieldInt(0, self.primeBase, self.primePower)
        C = [[fieldInt(0, self.primeBase, self.primePower, zero.prodPol) for __ in range(m)] for _ in range(n)]
        for i in range(n):
            for j in range(m):
                for k in range(p):
                    C[i][j] += self.matrix[i][k] * other.matrix[k][j]
                C[i][j] = C[i][j].value
        return fieldIntMatrix(C, self.primeBase, self.primePower)

    def __invert__(self):
        n, m = self.dim
        I = fieldIntMatrix.matrixEye(n, self.primeBase, self.primePower)
        if n != m:
            return I
